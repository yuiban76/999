...
function BingoTable({ state, signedIn, busy, onJoin }: { state: BingoState; signedIn: boolean; busy: boolean; onJoin: (entryFee?: number) => void }) {
  ...
  return <section className="casino-table bingo-table"><header><div><span>COMMUNITY BINGO</span><h3>多人共同開獎</h3><p>2～5 人 · 每人 NT${formatMoney(fee)} · 首位開房者設定報名費</p></div><strong>{state.status === "drawing" ? "開獎中" : state.status === "completed" ? "本輪結束" : "等待加入"}</strong></header><div className="bingo-draws"><span>已開出</span><div>{state.drawn.length ? state.drawn.map((number) => <b key={number}>{number}</b>) : <small>尚未開獎</small>}</div></div>{mine ? <div className="bingo-card">{mine.card.map((number) => <span className={state.drawn.includes(number) ? "marked" : ""} key={number}>{number}</span>)}</div> : <div className="tournament-join">{hosting && <label>報名費 <input type="number" min="100" max="10000" step="100" value={entryFee} onChange={(event) => setEntryFee(event.target.value)} /> <small>NT$100～10,000</small></label>}<button className="casino-primary" disabled={busy || !signedIn || state.status === "drawing" || state.players.length >= 5} onClick={() => onJoin(hosting ? Number(entryFee) : undefined)}>{signedIn ? hosting ? "開房並加入賓果" : `加入本輪賓果 · NT$${formatMoney(fee)}` : "登入後加入"}</button></div>}<div className="bingo-players">{state.players.map((player) => <p key={player.id}><strong>{player.displayName}{player.isMine ? "（你）" : ""}</strong><span>{player.card.filter((number) => state.drawn.includes(number)).length}/9 已標記</span></p>)}</div></section>;
}

function TournamentTable({ state, signedIn, busy, onJoin }: { state: TournamentState; signedIn: boolean; busy: boolean; onJoin: (game: "blackjack" | "poker", entryFee?: number) => void }) {
  ...
  return <section className="casino-table tournament-table"><header><div><span>CASINO TOURNAMENT</span><h3>{state.status === "lobby" ? "積分錦標賽" : `${gameName}積分賽`}</h3><p>2～5 人 · 報名費 NT${formatMoney(fee)} · 5 局積分後按名次分配獎金</p></div><strong>{state.status === "playing" ? `第 ${Math.min((state.currentRound ?? 0) + 1, state.roundLimit ?? 5)} 局` : state.status === "completed" ? "賽事結束" : "報名中"}</strong></header>{state.status === "lobby" && !joined && <div className="tournament-join">{hosting && <label>報名費 <input type="number" min="100" max="10000" step="100" value={entryFee} onChange={(event) => setEntryFee(event.target.value)} /> <small>NT$100～10,000</small></label>}<button disabled={busy || !signedIn} onClick={() => onJoin("blackjack", hosting ? Number(entryFee) : undefined)}>二十一點錦標賽 · NT${formatMoney(fee)}</button><button disabled={busy || !signedIn} onClick={() => onJoin("poker", hosting ? Number(entryFee) : undefined)}>德州撲克錦標賽 · NT${formatMoney(fee)}</button></div>}{state.latestResult && <p className="tournament-result">{state.latestResult}</p>}<ol className="tournament-rankings">{state.players.map((player, index) => <li key={player.id}><b>{index + 1}</b><span><strong>{player.displayName}{player.isMine ? "（你）" : ""}</strong><small>{player.latestHand || "等待開賽"}</small></span><em>{player.score} PTS</em></li>)}</ol>{state.status === "lobby" && joined && <p className="casino-message">已報名，等待至少一位玩家加入；玩法已鎖定為{gameName}，共 5 局。</p>}</section>;
}
